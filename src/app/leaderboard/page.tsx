...
// Build the data structure: model -> benchmark -> best score
function buildMatrix() {
  const matrix: Record<string, Record<string, { scoreLabel: string; score: number; isVendorReported: boolean }>> = {};
  ...
  ALL_BENCHMARKS.forEach((b) => {
    b.results.forEach((r) => {
      ...
      const existing = matrix[r.modelId][b.slug];
      if (!existing || r.score > existing.score) {
        matrix[r.modelId][b.slug] = {
...
export default function LeaderboardPage() {
  ...
  const visibleBenchmarks = useMemo(() => {
    ...
    if (categoryFilter !== "all") {
      benches = benches.filter((b) => b.category === categoryFilter);
    }
    return benches;
  }, [categoryFilter]);

  // Score each model for sorting (sum of scores across visible benchmarks)
  const sortedModels = useMemo(() => {
    return [...MODELS].sort((a, b) => {
      const aCount = visibleBenchmarks.filter((bm) => matrix[a.id]?.[bm.slug]).length;
      const bCount = visibleBenchmarks.filter((bm) => matrix[b.id]?.[bm.slug]).length;
  ...
  const benchmarkMaxes = useMemo(() => {
    ...
    visibleBenchmarks.forEach((b) => {
      const scores = MODELS.map((m) => matrix[m.id]?.[b.slug]?.score ?? 0);
      maxes[b.slug] = Math.max(...scores);
    });
    return maxes;
  }, [visibleBenchmarks, matrix]);
              ...
              {visibleBenchmarks.map((b) => (
                ...
                >
                  <Link
                    ...
                    title={b.name}
                  >
                    <span className="block max-w-[80px] truncate mx-auto">{b.name.split(" ")[0]}</span>
                  </Link>
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {sortedModels.map((model) => (
              ...
              >
                ...
                {/* Scores */}
                {visibleBenchmarks.map((b) => {
                  const entry = matrix[model.id]?.[b.slug];
                  return (
                    <td
                      key={b.slug}
                      ...
                      style={{
                        backgroundColor: entry
                          ? heatmapBg(entry.score, benchmarkMaxes[b.slug])
                          : "transparent",
                      }}
                    >
                      {entry ? (
